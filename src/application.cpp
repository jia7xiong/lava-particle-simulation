...
public:
    ...
    Particle(float m, vec3 p, vec3 v, vec3 c);
...
Particle::Particle(float m, vec3 p, vec3 v, vec3 c):mass(m),position(p),velocity(v),color(c),duration(0){}
...
void application::Add_Particles(int n)
{
    ...
    {
        float m = 1.0;
        vec3 p(random(-0.2, 0.2), 0.05 , random(-0.2, 0.2));
        vec3 v(10 * p[0], random(1.0, 10.0), 10 * p[2]);
        Particle particle(m, p, v, c);
...
vec3 Get_Particle_Color(float d)
{
    ...
    {
        vec3 yr = ((float)(1.5 - d)  * yellow + (float)(d - 0.1) * red) / (1.5 - 0.1);      
        return yr;
    }
    else if(d < 2) 
        return red;
...
void application::draw_event()
{
    ...
    if (!paused) {
        ...
        for(int i = 0; i< Particles.size(); i++) 
        {
            ...
            Particles[i].force += vec3(0 , -Particles[i].mass * g, 0);//+= =
            Particles[i].Euler_Step(h);
            Particles[i].Handle_Collision(damping, coeff_restitution);
        //
        //
    ...
    for(int i = 0; i < Particles.size(); i++)
    {
        ...
        vec3 p_odd = Particles[i].position + (float)0.04 * Particles[i].velocity;//
        glVertex3f(p_odd[0], p_odd[1], p_odd[2]);
        glEnd();
    }
